...
public class PlatformAuthController {
	...
	public ResponseEntity<LoginResponse> login(@Valid @RequestBody LoginRequest request, HttpServletRequest servletRequest) {
		return ResponseEntity.ok().cacheControl(CacheControl.noStore())
				.body(auth.platformLogin(request, AuditCallContext.correlationId(servletRequest)));
	}
	...
	public ResponseEntity<Void> changePassword(@Valid @RequestBody ChangePasswordRequest request,
		...
		auth.changePlatformPassword(current.principal().id(), request,
	...
	@SecurityRequirement(name = "bearerAuth")
	public ResponseEntity<PlatformContextResponse> context() {
		return ResponseEntity.ok().cacheControl(CacheControl.noStore())
				.body(auth.platformContext(current.principal().id()));
	}
}
